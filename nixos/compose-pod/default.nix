  ...
  podConfig = { name, config , ... }:
    {
      options = {
        ...
        envFile = mkOption {
          type = types.path;
          default = null;
          description = "Path to the environment file for the pod. (Will be loaded with systemd LoadCredentials, recommended to use e.g. a sops-nix secrets path)";
        };
      };
    };
...
in
{
  ...
  config = mkIf (eachPod != {}) {
    ...
    system.activationScripts = (mapAttrs' (podname: cfg: (
      let
        systemdDir = "/var/lib/${cfg.user}/.config/systemd/user";
      in
      nameValuePair "enable-pod-${podname}" { # here the systemd unit definition
        text = ''
          rm -rf "${systemdDir}/"
    ...
    systemd.user.services = (mapAttrs' (podname: cfg: (
      let homeDir = "/var/lib/${cfg.user}"; in
      nameValuePair "pod-${podname}" { # here the systemd unit definition
        ...
        wantedBy = [ /* "default.target" */ ]; # crappy UX, see https://github.com/NixOS/nixpkgs/issues/21460
    ...
    users.groups = mapAttrs' (podname: cfg: (
